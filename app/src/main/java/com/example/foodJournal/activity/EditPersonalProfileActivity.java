...
public class EditPersonalProfileActivity extends AppCompatActivity {
    ...
    private void do_edit_health_condition() {
        ...
        if (gender.getText().toString().equals("M")) {
            bmr_result = (int) Math.round((66 + 13.7 * weight_double + 5 * height_double - 6.8 * age_int) * 1.2);
        } else {
            bmr_result = (int) Math.round((655 + +9.6 * weight_double + 1.8 * height_double -4.7 * age_int) * 1.2);
        }
